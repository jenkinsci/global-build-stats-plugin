...
public abstract class AbstractBuildStatChartDimension {
    ...
    protected AbstractBuildStatChartDimension(
            BuildStatConfiguration config, DataSetBuilder<String, DateRange> datasetBuilder) {
        this.config = config;
    ...
    public static class BuildCounterChartDimension extends AbstractBuildStatChartDimension {
        ...
        public BuildCounterChartDimension(
                BuildStatConfiguration config, DataSetBuilder<String, DateRange> datasetBuilder) {
            super(config, datasetBuilder);
        }

        public void provideDataInDataSet(DateRange range) {
            config.getyAxisChartType()
                    .provideDataInDataSet(
                            datasetBuilder, range, nbSuccess, nbFailures, nbUnstables, nbAborted, nbNotBuild);
        ...
        public CategoryItemRenderer getRenderer() {
            ...
            final StackedAreaRenderer2 renderer = new StackedAreaRenderer2() {
                ...
                public String generateURL(CategoryDataset dataset, int row, int column) {
                    ...
                    boolean successShown = Messages.Build_Results_Item_Legend_Statuses_SUCCESS()
                            .equals(status);
                    boolean failuresShown = Messages.Build_Results_Item_Legend_Statuses_FAILURES()
                            .equals(status);
                    boolean unstablesShown = Messages.Build_Results_Item_Legend_Statuses_UNSTABLES()
                            .equals(status);
                    boolean abortedShown = Messages.Build_Results_Item_Legend_Statuses_ABORTED()
                            .equals(status);
                    boolean notBuildShown = Messages.Build_Results_Item_Legend_Statuses_NOT_BUILD()
                            .equals(status);

                    StringBuilder sb = new StringBuilder()
                            .append("buildHistory?jobFilter=")
                            .append(URLEncoder.encode(config.getBuildFilters().getJobFilter()))
                            .append("&start=")
                            .append(range.getStart().getTimeInMillis())
                            .append("&end=")
                            .append(range.getEnd().getTimeInMillis())
                            .append("&successShown=")
                            .append(successShown)
                            .append("&failuresShown=")
                            .append(failuresShown)
                            .append("&unstablesShown=")
                            .append(unstablesShown)
                            .append("&abortedShown=")
                            .append(abortedShown)
                            .append("&notBuildShown=")
                            .append(notBuildShown);
                    if (config.getBuildFilters().getNodeFilter() != null) {
                        sb.append("&nodeFilter=")
                                .append(URLEncoder.encode(
                                        config.getBuildFilters().getNodeFilter()));
                    }
                    if (config.getBuildFilters().getLauncherFilter() != null) {
                        sb.append("&launcherFilter=")
                                .append(URLEncoder.encode(
                                        config.getBuildFilters().getLauncherFilter()));
                    }
                    return sb.toString();
                }
    ...
    public static class BuildTotalDurationChartDimension extends AbstractBuildStatChartDimension {
        ...
        public BuildTotalDurationChartDimension(
                BuildStatConfiguration config, DataSetBuilder<String, DateRange> datasetBuilder) {
            super(config, datasetBuilder);
        }
        ...
        public void provideDataInDataSet(DateRange range) {
            datasetBuilder.add(
                    Long.valueOf(totalBuildDuration).doubleValue() / 1000.0,
                    Messages.Build_Results_Total_Build_Time(),
                    range);
    ...
    public static class BuildAverageDurationChartDimension extends AbstractBuildStatChartDimension {
        ...
        public BuildAverageDurationChartDimension(
                BuildStatConfiguration config, DataSetBuilder<String, DateRange> datasetBuilder) {
            super(config, datasetBuilder);
        }
        ...
        public void provideDataInDataSet(DateRange range) {
            ...
            datasetBuilder.add(
                    buildCounter == 0 ? 0 : Long.valueOf(totalBuildDuration).doubleValue() / (buildCounter * 1000.0),
                    Messages.Build_Results_Average_Build_Time(),
                    range);
