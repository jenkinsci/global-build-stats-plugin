...
public class JobBuildResultSharder {
    ...
    private final List<JobBuildResult> queuedResultsToAdd =
            Collections.synchronizedList(new ArrayList<JobBuildResult>());
    ...
    public void applyQueuedResultsInFiles() {
        ...
        if (!jobResultsRoot.exists()) {
            try {
                ...
                throw new IllegalStateException(
                        "Can't create job results root directory : " + jobResultsRoot.getAbsolutePath(), e);
            }
        }
        ...
        List<String> updatedFilenamesList =
                new ArrayList<String>(toJobResultFilenameMap(resultsToRemove).keySet());
    ...
    private static Map<String, List<JobBuildResult>> toJobResultFilenameMap(List<JobBuildResult> results) {
        ...
        for (JobBuildResult r : results) {
            String targetFilename =
                    "jobResults-" + yearMonth.format(r.getBuildDate().getTime()) + ".xml";
