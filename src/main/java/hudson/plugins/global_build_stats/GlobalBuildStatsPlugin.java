...
public class GlobalBuildStatsPlugin extends Plugin {
    ...
    public static class GlobalBuildStatsApi extends Api {
        ...
        private static boolean exposeChartData(StaplerRequest2 req, StaplerResponse2 rsp, Flavor flavor)
            ...
            if (buildStatConfigId != null) {
                BuildStatConfiguration config =
                        GlobalBuildStatsPlugin.getPluginBusiness().searchBuildStatConfigById(buildStatConfigId);
                if (config != null) {
                    List<AbstractBuildStatChartDimension> dimensions =
                            GlobalBuildStatsPlugin.getPluginBusiness().createDataSetBuilder(config);
    ...
    public void doShowChart(StaplerRequest2 req, StaplerResponse2 res) throws ServletException, IOException {
        ...
        ChartUtil.generateGraph(
                org.kohsuke.stapler.StaplerRequest.fromStaplerRequest2(req),
    ...
    public void doBuildHistory(StaplerRequest2 req, StaplerResponse2 res) throws ServletException, IOException {
        ...
        req.getView(this, "/hudson/plugins/global_build_stats/GlobalBuildStatsPlugin/buildHistory.jelly")
                .forward(req, res);
    }

    @RequirePOST
    public void doUpdateBuildStatConfiguration(StaplerRequest2 req, StaplerResponse2 res)
            throws ServletException, IOException {
        Hudson.getInstance().checkPermission(getRequiredPermission());
        ...
        BuildStatConfiguration config =
                FromRequestObjectFactory.createBuildStatConfiguration(req.getParameter("buildStatId"), req);
    ...
    @RequirePOST
    public void doAddBuildStatConfiguration(StaplerRequest2 req, StaplerResponse2 res)
            throws ServletException, IOException {
        Hudson.getInstance().checkPermission(getRequiredPermission());

        BuildStatConfiguration config = FromRequestObjectFactory.createBuildStatConfiguration(
                ModelIdGenerator.INSTANCE.generateIdForClass(BuildStatConfiguration.class), req);
    ...
    @RequirePOST
    public void doUpdateRetentionStrategies(StaplerRequest2 req, StaplerResponse2 res)
            throws ServletException, IOException {
        Hudson.getInstance().checkPermission(getRequiredPermission());
